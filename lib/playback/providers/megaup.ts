/**
 * lib/playback/providers/megaup.ts
 *
 * MegaUp Video Host Adapter
 * Configuration:
 *   - MEGAUP_ENABLED=false (default)
 */
...
export class MegaUpProvider implements PlaybackProvider {
  id = "megaup";
  name = "MegaUp";
  enabled = process.env.MEGAUP_ENABLED === "true";
  priority = 22;
  ...
  async healthCheck(): Promise<{
    ...
    if (!this.enabled) {
      return { status: "DISABLED", message: "MEGAUP_ENABLED is false" };
    }
    return { status: "ACTIVE", message: "MegaUp slot configured" };
  }
  ...
  async resolve(request: PlaybackRequest): Promise<PlaybackCandidate | null> {
    ...
    const source = mapped.find((m) => m.providerId.toLowerCase() === "megaup");
