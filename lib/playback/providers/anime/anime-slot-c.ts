/**
 * lib/playback/providers/anime/anime-slot-c.ts
 *
 * Configurable Anime Provider C Adapter
 * Environment variables:
 *   - ANIME_PROVIDER_C_ENABLED=false
 *   - ANIME_PROVIDER_C_API_URL=
 *   - ANIME_PROVIDER_C_API_KEY=
 */
...
export class AnimeProviderC implements PlaybackProvider {
  id = "anime-provider-c";
  name = "Anime Server Gamma";
  enabled = process.env.ANIME_PROVIDER_C_ENABLED === "true";
  ...
  private getBaseUrl(): string | undefined {
    return process.env.ANIME_PROVIDER_C_API_URL?.trim();
  }

  private getApiKey(): string | undefined {
    return process.env.ANIME_PROVIDER_C_API_KEY?.trim();
  }
  ...
  async healthCheck(): Promise<{
    ...
    if (!this.enabled) {
      return { status: "DISABLED", message: "ANIME_PROVIDER_C_ENABLED is false" };
    }
    const baseUrl = this.getBaseUrl();
    if (!baseUrl) {
      return { status: "NOT_CONFIGURED", message: "ANIME_PROVIDER_C_API_URL is missing" };
    }
    ...
    try {
      ...
      return {
        ...
        latencyMs,
        message: `Anime Server Gamma endpoint responsive (${latencyMs}ms)`,
      };
    } catch (err: any) {
      return {
        status: "FAILED",
  ...
  async resolveAnime(request: AnimePlaybackRequest): Promise<PlaybackCandidate | null> {
    ...
    return {
      ...
      quality: "1080p",
      serverLabel: `Anime Gamma (${isDub ? "DUB" : "SUB"})`,
      serverNumber: 4,
      statusText: "Anime Server Gamma — Ready",
